...
public abstract class UIBasicSprite : UIWidget
{
    ...
    [HideInInspector]
    [SerializeField]
    protected Flip mMirror = Flip.Nothing;
    [HideInInspector]
    [SerializeField]
    protected bool mFillBorder = true;
    [HideInInspector]
    [SerializeField]
    protected bool mTileBorder = false;
    ...
    void SimpleFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
    {
        ...
        {
            if (!mFillBorder || !hasBorder) return;
        ...
        if (mFillDirection == FillDirection.Horizontal)
        {
            if (mInvert)
            {
                fillBounds[0].x = Mathf.Lerp(mTempPos[partX].x, mTempPos[0].x, mFillAmount);
                fillBounds[1].x = mTempPos[partX].x;
            }
            else
            {
                fillBounds[0].x = mTempPos[0].x;
                fillBounds[1].x = Mathf.Lerp(mTempPos[0].x, mTempPos[partX].x, mFillAmount);
            }
            fillBounds[0].y = mTempPos[0].y;
            fillBounds[1].y = mTempPos[partY].y;
        }
        else if (mFillDirection == FillDirection.Vertical)
        {
            if (mInvert)
            {
                fillBounds[0].y = Mathf.Lerp(mTempPos[partY].y, mTempPos[0].y, mFillAmount);
                fillBounds[1].y = mTempPos[partY].y;
            }
            else
            {
                fillBounds[0].y = mTempPos[0].y;
                fillBounds[1].y = Mathf.Lerp(mTempPos[0].y, mTempPos[partY].y, mFillAmount);
            }
            fillBounds[0].x = mTempPos[0].x;
            fillBounds[1].x = mTempPos[partX].x;
        }
        ...
        {
            for (int x1 = 0, x2 = x1 + 1; x1 < partX; x2 = ++x1 + 1)
            {
                for (int y1 = 0, y2 = y1 + 1; y1 < partY; y2 = ++y1 + 1)
                {
                    if (centerType == AdvancedType.Invisible && x1 > 0 && x2 < partX && y1 > 0 && y2 < partY)
                        continue;
